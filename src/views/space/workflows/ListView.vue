...
import { useDeleteWorkflow, useGetWorkflowsWithPage } from '@/hooks/use-workflow.ts'
...
onMounted(async () => {
  // 初始化工作流数据数据
  await loadWorkflows((route.query?.search_word as string) ?? '', '', true)
})
...
watch(
  ...
  async () => await loadWorkflows((route.query?.search_word as string) ?? '', '', true),
)
</script>
