...
import { useGetPublishHistoriesWithPage, useFallbackHistoryToDraft } from '@/hooks/use-app.ts'
...
const handleScroll = async (event: UIEvent) => {
  ...
  if (scrollTop + clientHeight >= scrollHeight - 10) {
    ...
    await loadPublishHistories(props.app?.id as string)
  }
}
...
watch(
  ...
  async (newValue) => {
    if (newValue) {
      await loadPublishHistories(props.app?.id as string, true)
    } else {
        ...
        <div class="flex items-center gap-3 mb-3">
          ...
          <div class="flex flex-col">
            ...
            <div class="text-xs text-gray-500">
              最近编辑 · {{ moment(app?.draft_updated_at).format('YYYY-MM-DD HH:mm:ss') }}
            </div>
          </div>
        </div>
        <!-- 应用描述 -->
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="text-xs text-gray-500">
              发布时间：{{ moment(publishHistory.created_at).format('YYYY-MM-DD HH:mm') }}
            </div>
          </div>
          <!-- 回退按钮 -->
          <a-button
